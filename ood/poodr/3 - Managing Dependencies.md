...
An object dependes on another if, when one changes, the other might
be forced to change in turn. [connascence.md]

## Recognizing Dependences

An object has a dependecy when it knows:
...
stick to the things it touches. A few dots are necessayr, but apply too
...
should be consice, explicit, and isolated.
...
naturally decoupled;
  ...
  * Isolate vunlerable external messages:
...
This technique becomes necessary when a class containe embedded references
...
_Depend on things that change less often than you_.
  ...
  * changing a class that has many dependents will resul in
...
thoughfully designed application this is inevitable: dependencies
